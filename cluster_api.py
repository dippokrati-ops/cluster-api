...
def geocode_address(address):
    try:
        ...
        print(f"Nominatim response: {r.text[:500]}")
        data = r.json()
...
def lookup():
    ...
    if row is None:
        return jsonify({"error": "No cluster found", "lat": lat, "lng": lng}), 404
...
def test_geocode():
    """Test endpoint to debug geocoding"""
    try:
        headers = {"User-Agent": "ClusterAPI/1.0 contact@skroutzdelivery.gr"}
        r = requests.get(
            "https://nominatim.openstreetmap.org/search",
            params={"q": "Glyfada, Greece", "format": "json", "limit": 1},
            headers=headers,
            timeout=10
        )
